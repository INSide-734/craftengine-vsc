...
describe('EventBus Performance', () => {
    ...
    describe('Subscription Operations', () => {
        bench('subscribe single handler', () => {
        ...
        bench('subscribe 10 handlers to same event', () => {
            const eventBus = new EventBus(mockLogger);
        ...
        bench('subscribe 100 handlers to different events', () => {
            const eventBus = new EventBus(mockLogger);
            const subs: IEventSubscription[] = [];
            for (let i = 0; i < 100; i++) {
        ...
        bench('subscribe with wildcard pattern', () => {
            const eventBus = new EventBus(mockLogger);
    ...
    describe('Subscription Query', () => {
        ...
        bench('getSubscriptionCount (non-existent)', () => {
            const eventBus = new EventBus(mockLogger);
            eventBus.getSubscriptionCount('non.existent.event');
            eventBus.dispose();
        }, defaultBenchOptions);

        bench('check subscription isActive', () => {
            const eventBus = new EventBus(mockLogger);
            const sub = eventBus.subscribe('test.event', () => {});
            sub.isActive();
            eventBus.dispose();
    ...
    describe('Batch Subscription Operations', () => {
        bench('subscribe and unsubscribe 50 handlers', () => {
            const eventBus = new EventBus(mockLogger);
            const subs: IEventSubscription[] = [];
            for (let i = 0; i < 50; i++) {
        ...
        bench('unsubscribeAll for event type', () => {
            const eventBus = new EventBus(mockLogger);
            for (let i = 0; i < 10; i++) {
    ...
    describe('Event Pattern Matching (Simulated)', () => {
        ...
        const eventTypes = [
            ...
            'translation.added', 'translation.removed',
            'cache.invalidated', 'config.changed',
            'file.changed', 'file.deleted', 'system.ready'
        ];
        ...
        bench('wildcard match (10 event types)', () => {
        ...
        bench('regex match (10 event types)', () => {
            const pattern = /^template\./;
            for (const eventType of eventTypes) {
        ...
        bench('multiple pattern check (5 patterns x 10 events)', () => {
    ...
    describe('Memory Efficiency', () => {
        bench('create and dispose event bus', () => {
            const bus = new EventBus(mockLogger);
            bus.dispose();
        }, defaultBenchOptions);

        bench('create event bus with 100 subscriptions then dispose', () => {
            const bus = new EventBus(mockLogger);
            for (let i = 0; i < 100; i++) {
    ...
    describe('CraftEngine Usage Patterns', () => {
        bench('typical subscription setup', () => {
            const bus = new EventBus(mockLogger);
        ...
        bench('rapid subscribe/unsubscribe cycle', () => {
            const eventBus = new EventBus(mockLogger);
