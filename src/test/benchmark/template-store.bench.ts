...
describe('TemplateStore Performance (Data Structure)', () => {
    ...
    describe('Map Operations - Query (1000 templates)', () => {
        ...
        bench(
            'filter by pattern in 1000',
            () => {
                Array.from(templatesMap.values()).filter((t) => t.name.startsWith('default:model/'));
            },
            fastBenchOptions,
        );
    });
