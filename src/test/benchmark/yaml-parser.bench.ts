...
function generateFullItemConfig(itemCount: number): string {
    ...
    const itemNames = ['jade_sword', 'phoenix_staff', 'dragon_blade', 'thunder_axe', 'frost_pickaxe', 
                       'flame_helmet', 'shadow_armor', 'light_boots', 'nature_ring', 'void_amulet'];
...
function generateArmorTrimTemplates(templateCount: number): string {
    const templates: string[] = [];
    const trims = ['quartz', 'iron', 'netherite', 'redstone', 'copper', 'gold', 'emerald', 'diamond', 'lapis', 'amethyst'];
    
    for (let i = 0; i < templateCount; i++) {
        const cases = trims.map((trim) => `
        - when: minecraft:${trim}
          model:
            type: minecraft:model
            ...
            generation:
              ...
              textures:
                ...
                layer1: minecraft:trims/items/helmet_trim_${trim}`).join('');
...
describe('YamlParser Performance - CraftEngine Style', () => {
    ...
    describe('Items Configuration Parsing', () => {
        bench('parse 10 items (template mode)', () => {
            yaml.parse(smallItems);
        }, defaultBenchOptions);

        bench('parse 50 items (template mode)', () => {
            yaml.parse(mediumItems);
        }, defaultBenchOptions);

        bench('parse 100 items (template mode)', () => {
            yaml.parse(largeItems);
        }, fastBenchOptions);
    });

    describe('Full Item Config Parsing', () => {
        bench('parse 10 items (full config with data/components)', () => {
            yaml.parse(fullItemsSmall);
        }, defaultBenchOptions);

        bench('parse 30 items (full config)', () => {
            yaml.parse(fullItemsMedium);
        }, fastBenchOptions);

        bench('parse 50 items (full config)', () => {
            yaml.parse(fullItemsLarge);
        }, fastBenchOptions);
    });

    describe('Version Conditional Items', () => {
        bench('parse 20 version-conditional items', () => {
    ...
    describe('Model Templates Parsing', () => {
        bench('parse 10 simple model templates', () => {
            yaml.parse(modelTemplates10);
        }, defaultBenchOptions);

        bench('parse 30 simple model templates', () => {
            yaml.parse(modelTemplates30);
        }, defaultBenchOptions);

        bench('parse 10 complex bow-style templates', () => {
            yaml.parse(complexModels10);
        }, defaultBenchOptions);

        bench('parse 5 armor trim templates (select/cases)', () => {
    ...
    describe('Translations Parsing', () => {
        bench('parse translations (20 keys × 4 languages)', () => {
            yaml.parse(translations20);
        }, defaultBenchOptions);

        bench('parse translations (50 keys × 4 languages)', () => {
    ...
    describe('Categories Parsing', () => {
        bench('parse 10 categories (100 items total)', () => {
    ...
    describe('Full Configuration File Parsing', () => {
        bench('parse small config (20 items, 10 templates, 20 translations)', () => {
            yaml.parse(fullConfigSmall);
        }, defaultBenchOptions);

        bench('parse medium config (50 items, 20 templates, 40 translations)', () => {
            yaml.parse(fullConfigMedium);
        }, fastBenchOptions);

        bench('parse large config (100 items, 30 templates, 60 translations)', () => {
    ...
    describe('Special Format Parsing', () => {
        bench('parse sectioned config (templates#section#block format)', () => {
            yaml.parse(sectionedConfig);
        }, defaultBenchOptions);

        bench('parse MiniMessage rich text items', () => {
    ...
    describe('Parse vs ParseDocument Comparison', () => {
        bench('yaml.parse (medium config)', () => {
            yaml.parse(fullConfigMedium);
        }, defaultBenchOptions);

        bench('yaml.parseDocument (medium config)', () => {
            yaml.parseDocument(fullConfigMedium);
        }, defaultBenchOptions);

        bench('yaml.parseAllDocuments (medium config)', () => {
    ...
    describe('Strict Mode Comparison', () => {
        bench('parse with default options', () => {
            yaml.parse(fullItemsMedium);
        }, defaultBenchOptions);

        bench('parse with strict schema', () => {
