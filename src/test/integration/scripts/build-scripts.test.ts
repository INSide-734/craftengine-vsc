...
describe('Build Scripts Integration Tests', () => {
  ...
  describe('verify-build.js', () => {
    ...
    it('should fail when out directory exists but dependencies are missing', () => {
      ...
      const verifyBuildScript = readFileSync(
        join(scriptsDir, 'verify-build.js'),
        'utf-8'
      );
...
`
      ...
      const output = execSync(
        `node scripts/generate-release-notes.js 1.0.0 "${tempChangelog}"`,
        {
          cwd: rootDir,
          stdio: 'pipe',
          encoding: 'utf-8',
  ...
  describe('analyze-dependencies.js', () => {
    it('should analyze package dependencies', async () => {
      ...
      const { analyzeMultiplePackages } = await import(
        join(scriptsDir, 'analyze-dependencies.js')
      );
