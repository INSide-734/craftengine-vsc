...
describe('MinecraftDataLoader', () => {
    ...
    const mockEnchantmentsResponse = [
        {
            id: 16, name: 'sharpness', displayName: 'Sharpness', maxLevel: 5,
            category: 'weapon', treasureOnly: false, curse: false, weight: 10,
            exclude: ['smite', 'bane_of_arthropods']
        },
        {
            id: 17, name: 'smite', displayName: 'Smite', maxLevel: 5,
            category: 'weapon', treasureOnly: false, curse: false, weight: 5,
    ...
    const mockBlocksResponse = [
        {
            id: 1, name: 'stone', displayName: 'Stone', hardness: 1.5, resistance: 6,
            stackSize: 64, diggable: true, transparent: false, emitLight: 0, filterLight: 15, boundingBox: 'block'
        },
        {
            id: 2, name: 'grass_block', displayName: 'Grass Block', hardness: 0.6, resistance: 0.6,
            stackSize: 64, diggable: true, transparent: false,
    ...
    const mockItemsResponse = [
        ...
        {
            id: 802, name: 'diamond_sword', displayName: 'Diamond Sword', stackSize: 1,
            durability: 1561, enchantCategories: ['weapon', 'breakable'], repairWith: ['diamond']
        }
    ];
    ...
    beforeEach(() => {
        ...
        mockVersionService = {
            ...
            getVersions: vi.fn(), refresh: vi.fn(), isValidVersion: vi.fn(),
            compareVersions: vi.fn(), isValidVersionFormat: vi.fn(), getSuggestedVersions: vi.fn()
        };
        ...
        const mockConfigLoader = {
            ...
            loadTimingConfig: vi.fn(), loadDataSourcesConfig: vi.fn()
        };

        vi.spyOn(ServiceContainer, 'getService').mockImplementation((token: string | symbol) => {
            if (token === SERVICE_TOKENS.Logger) {return mockLogger as unknown;}
            if (token === SERVICE_TOKENS.MinecraftVersionService) {return mockVersionService as unknown;}
            if (token === SERVICE_TOKENS.DataConfigLoader) {return mockConfigLoader as unknown;}
