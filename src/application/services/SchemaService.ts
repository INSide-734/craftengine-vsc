...
export class SchemaService implements ISchemaService {
     ...
     * ```
     */
    async registerSchemaProvider(context: ExtensionContext): Promise<void> {
    ...
    async resolveReferences(schema: IJsonSchema, maxDepth: number = 5): Promise<IJsonSchema> {
        ...
        return await resolver.resolveReferences(schema as unknown as JsonSchemaNode, maxDepth) as unknown as IJsonSchema;
    }
    ...
    async getTopLevelFields(): Promise<string[]> {
        ...
        return await this.queryService.getTopLevelFields(rootSchema);
    }
    ...
    async getAvailableProperties(path: string[]): Promise<SchemaProperty[]> {
        ...
        return await this.queryService.getAvailableProperties(rootSchema, path);
    }
    ...
    async getPropertyDetails(path: string[]): Promise<SchemaPropertyDetails | undefined> {
        ...
        return await this.queryService.getPropertyDetails(rootSchema, path);
    }
     ...
     * 初始化子服务
     */
    private async initializeSubServices(context: ExtensionContext): Promise<void> {
        // 创建加载服务
        ...
        this.queryService = new SchemaQueryService(
            navigator,
            this.extractor,
            this.logger,
            this.performanceMonitor
