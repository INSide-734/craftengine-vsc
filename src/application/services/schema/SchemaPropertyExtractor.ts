...
import { SchemaReferenceResolver, SchemaProperty, SchemaPropertyDetails, isCircularRef } from './index';
...
export class SchemaPropertyExtractor {
    ...
    async extractProperties(schema: JsonSchemaNode, contextSchema?: JsonSchemaNode): Promise<SchemaProperty[]> {
        ...
        // 从 properties 提取
        await this.extractFromProperties(schema, properties);
        
        // 从 patternProperties 提取
        await this.extractFromPatternProperties(schema, properties);
        ...
        if (!result && patternProperties) {
            for (const [pattern, schema] of Object.entries(patternProperties)) {
                try {
                    ...
                    this.logger.debug('Invalid regex pattern', { pattern });
                }
            }
        }
     ...
     * 从 properties 提取
     */
    private async extractFromProperties(
     ...
     * 从 patternProperties 提取
     */
    private async extractFromPatternProperties(
        schema: JsonSchemaNode,
        properties: SchemaProperty[]
    ): Promise<void> {
        ...
        for (const subSchema of schema.allOf as JsonSchemaNode[]) {
            ...
            if (subProperties) {
                for (const [key, propSchema] of Object.entries(subProperties)) {
                    if (typeof propSchema === 'object' && propSchema !== null && !properties.find(p => p.key === key)) {
                        properties.push({ key, schema: propSchema as JsonSchemaNode });
                    }
                }
            }
