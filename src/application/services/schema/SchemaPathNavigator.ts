...
export class SchemaPathNavigator {
    ...
    async getSchemaForPath(rootSchema: JsonSchemaNode, path: string[]): Promise<JsonSchemaNode | undefined> {
        ...
        for (let i = 0; i < path.length; i++) {
            ...
            // 解析引用，使用当前上下文 Schema
            const resolvedSchema = await this.resolver.resolveReferences(
                currentSchema,
                SCHEMA_RESOLUTION.DEFAULT_MAX_DEPTH,
            ...
            if (!nextSchema[SCHEMA_METADATA.CONTEXT_SCHEMA] && currentContextSchema) {
                ...
                const effectiveSchemaDir =
                    resolvedSchema?.[SCHEMA_METADATA.SCHEMA_DIR] || currentContextSchema[SCHEMA_METADATA.SCHEMA_DIR];
                const effectiveSchemaFile =
                    resolvedSchema?.[SCHEMA_METADATA.SCHEMA_FILE] || currentContextSchema[SCHEMA_METADATA.SCHEMA_FILE];
        ...
        // 最后再解析一次引用，使用当前上下文
        return this.resolver.resolveReferences(
            currentSchema,
            SCHEMA_RESOLUTION.DEFAULT_MAX_DEPTH,
            currentContextSchema,
        );
