...
export class TemplateExpander implements ITemplateExpander {
    ...
    private async expandObjectInternal(
        ...
        if (Array.isArray(obj)) {
            ...
            for (let i = 0; i < obj.length; i++) {
                result.push(await this.expandObjectInternal(
                    obj[i],
                    [...path, String(i)],
                    visited,
                    positionMap,
        ...
        if ('template' in objRecord && objRecord.template) {
            return await this.expandWithTemplate(
                objRecord,
                path,
                visited,
                positionMap,
    ...
    private async expandWithTemplate(
        ...
        const templateNames = Array.isArray(templateField) 
            ? templateField as string[]
            : [templateField as string];
        ...
        if (content && typeof content === 'object') {
            ...
            for (const [key, value] of Object.entries(content as Record<string, unknown>)) {
                ...
                const newKey = typeof key === 'string'
                    ? this.substituteStringParameters(key, params)
                    : key;
     ...
     * 替换字符串中的参数占位符
     */
    private substituteStringParameters(
        str: string,
        params: Record<string, unknown>
    ): string {
        return str.replace(
