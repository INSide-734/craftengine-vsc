...
export class MiniMessageCodeActionProvider implements CodeActionProvider, Disposable {
    ...
    static readonly providedCodeActionKinds = [CodeActionKind.QuickFix, CodeActionKind.Refactor];

    constructor() {
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger).createChild(
            'MiniMessageCodeActionProvider',
        );
    ...
    async provideCodeActions(
        ...
        try {
            ...
            const miniMessageDiagnostics = context.diagnostics.filter((d) => d.source?.includes('MiniMessage'));

            for (const diagnostic of miniMessageDiagnostics) {
                const diagnosticActions = this.createActionsForDiagnostic(document, diagnostic);
     ...
     * 为诊断创建修复操作
     */
    private createActionsForDiagnostic(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];
     ...
     * 创建未闭合标签修复
     */
    private createUnclosedTagFixes(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];
        ...
        // 修复方案 1：在行尾添加关闭标签
        const fix1 = new CodeAction(`Add closing tag '</${tagName}>'`, CodeActionKind.QuickFix);
        ...
        // 修复方案 2：将标签改为自闭合
        const fix2 = new CodeAction(`Convert to self-closing tag '<${tagName}/>'`, CodeActionKind.QuickFix);
     ...
     * 创建无效标签修复
     */
    private createInvalidTagFixes(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];
        ...
        if (messageMatch) {
            ...
            for (const suggestion of suggestions) {
                ...
                const fix = new CodeAction(`Change to '<${trimmedSuggestion}>'`, CodeActionKind.QuickFix);
     ...
     */
    private createInvalidColorFixes(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];
        const tagText = document.getText(diagnostic.range);
        ...
        for (const colorName of colorNames.slice(0, 5)) {
            const fix = new CodeAction(`Use color '${colorName}'`, CodeActionKind.QuickFix);

            // 替换颜色参数
            const newTagText = tagText.replace(/<(color|colour|c):([^>]+)>/i, `<$1:${colorName}>`);
        ...
        // 提供十六进制颜色模板
        const hexFix = new CodeAction(`Use hex color '#FF5555'`, CodeActionKind.QuickFix);

        const hexTagText = tagText.replace(/<(color|colour|c):([^>]+)>/i, '<$1:#FF5555>');
     ...
     */
    private createInvalidClickActionFixes(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];
        const tagText = document.getText(diagnostic.range);

        for (const action of this.dataLoader.getClickActions()) {
            const fix = new CodeAction(`Use click action '${action}'`, CodeActionKind.QuickFix);

            const newTagText = tagText.replace(/<click:([^:>]+)/i, `<click:${action}`);
     ...
     */
    private createInvalidHoverActionFixes(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];
        const tagText = document.getText(diagnostic.range);

        for (const action of this.dataLoader.getHoverActions()) {
            const fix = new CodeAction(`Use hover action '${action}'`, CodeActionKind.QuickFix);

            const newTagText = tagText.replace(/<hover:([^:>]+)/i, `<hover:${action}`);
     ...
     * 创建无匹配关闭标签修复
     */
    private createUnmatchedClosingTagFixes(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];

        // 删除无匹配的关闭标签
        const fix = new CodeAction('Remove unmatched closing tag', CodeActionKind.QuickFix);
        fix.edit = new WorkspaceEdit();
     ...
     * 创建缺失参数修复
     */
    private createMissingArgumentFixes(document: TextDocument, diagnostic: Diagnostic): CodeAction[] {
        const actions: CodeAction[] = [];
        const tagText = document.getText(diagnostic.range);
        ...
        const fix = new CodeAction(`Replace with template: ${template}`, CodeActionKind.QuickFix);
        fix.edit = new WorkspaceEdit();
     ...
     * 创建删除标签操作
     */
    private createDeleteTagAction(document: TextDocument, diagnostic: Diagnostic): CodeAction {
        const fix = new CodeAction('Remove this tag', CodeActionKind.QuickFix);
        fix.edit = new WorkspaceEdit();
        fix.edit.delete(document.uri, diagnostic.range);
        fix.diagnostics = [diagnostic];
        return fix;
     ...
     * 创建重构操作
     */
    private createRefactorActions(document: TextDocument, range: Range): CodeAction[] {
        const actions: CodeAction[] = [];
        ...
        if (selectedText.length > 0 && !selectedText.includes('\n')) {
            // 用颜色包装
            actions.push(
                this.createWrapAction(document, range, selectedText, 'Wrap with color tag', '<red>', '</red>'),
            );

            // 用粗体包装
            actions.push(
                this.createWrapAction(document, range, selectedText, 'Wrap with bold tag', '<bold>', '</bold>'),
            );

            // 用斜体包装
            actions.push(
                this.createWrapAction(document, range, selectedText, 'Wrap with italic tag', '<italic>', '</italic>'),
            );
            ...
            // 用彩虹包装
            actions.push(
                this.createWrapAction(document, range, selectedText, 'Wrap with rainbow', '<rainbow>', '</rainbow>'),
            );
