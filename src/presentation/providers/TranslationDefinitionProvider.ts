...
export class TranslationDefinitionProvider implements DefinitionProvider {
    ...
    constructor() {
        ...
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger)
            .createChild('TranslationDefinitionProvider');
    ...
    async provideDefinition(
        ...
        try {
            ...
            // 搜索翻译定义
            const definitions = await this.findTranslationDefinitions(
                translationInfo.key,
                translationInfo.range
            );
            ...
            const locations = definitions.map(def => 
                new Location(def.targetUri, def.targetRange)
            );
        ...
        while ((match = pattern.exec(lineText)) !== null) {
            ...
            if (position.character >= startPos && position.character <= endPos) {
                ...
                const range = new Range(
                    position.line, keyStartPos,
                    position.line, keyEndPos
                );
     ...
     * 查找翻译定义位置
     */
    private async findTranslationDefinitions(
        keyName: string,
        originRange: Range
     ...
     * 创建 LocationLink
     */
    private async createLocationLink(
        key: ITranslationKey,
        originRange: Range
        ...
        try {
            ...
            // 尝试获取更精确的位置
            const preciseLocation = await this.findPreciseLocation(
                targetUri, 
                key.key, 
                key.languageCode,
                lineNumber
    ...
    private async searchEntireDocument(
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (trimmed.startsWith('translations:') || 
