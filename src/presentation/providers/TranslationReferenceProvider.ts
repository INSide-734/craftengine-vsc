...
export class TranslationReferenceProvider implements ReferenceProvider {
    ...
    async provideReferences(
        ...
        try {
            ...
            // 搜索所有引用
            const locations = await this.findAllReferences(translationInfo.key, context.includeDeclaration);
    ...
    private isInTranslationsSection(document: TextDocument, lineNumber: number): boolean {
        ...
        for (let i = lineNumber - 1; i >= 0; i--) {
            ...
            if (text.match(/^[a-z][a-z0-9_-]+:/) && !text.match(/^([a-z]{2}(_[a-z]{2})?):/)) {
                if (
                    text.startsWith('translations:') ||
     ...
     * 查找翻译键的所有引用（基于内存索引）
     */
    private async findAllReferences(keyName: string, includeDeclaration: boolean): Promise<Location[]> {
