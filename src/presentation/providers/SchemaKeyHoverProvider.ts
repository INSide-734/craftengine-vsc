...
export class SchemaKeyHoverProvider implements HoverProvider {
    ...
    private appendBadges(md: MarkdownString, details: { required?: boolean; deprecated?: boolean }): void {
        const badges: string[] = [];
        if (details.required) { badges.push('`required`'); }
        if (details.deprecated) { badges.push('~~`deprecated`~~'); }
        if (badges.length > 0) {
    ...
    private appendEnumValues(md: MarkdownString, details: { enum?: unknown[] }): void {
        if (!details.enum || details.enum.length === 0) { return; }
    ...
    private appendDefaultValue(md: MarkdownString, details: { default?: unknown }): void {
        if (details.default === undefined) { return; }
    ...
    /** 追加示例 */
    private appendExamples(md: MarkdownString, key: string, details: { examples?: unknown[]; type?: string | string[]; enum?: unknown[]; default?: unknown }): void {
