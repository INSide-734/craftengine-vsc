...
import { ICompletionContextInfo } from '../../core/interfaces/ICompletionStrategy';
...
export class BaseCompletionProvider implements CompletionItemProvider {
    ...
    constructor() {
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger)
            .createChild('BaseCompletionProvider');
        ...
        this.completionManager = ServiceContainer.getService<CompletionManager>(
            SERVICE_TOKENS.CompletionManager
        );
        ...
        try {
            ...
            const results = await Promise.allSettled(
                activeStrategies.map(strategy =>
                    strategy.provideCompletionItems(completionContext, token)
                )
            );
    ...
    async resolveCompletionItem(
        ...
        try {
            ...
            // 查找对应的策略
            const strategy = this.completionManager.getStrategies()
