...
import { type IFilePathDiagnosticData } from './FilePathDiagnosticProvider';
...
export class FilePathCodeActionProvider implements CodeActionProvider {
    ...
    async provideCodeActions(
        ...
        try {
            ...
            for (const diagnostic of filePathDiagnostics) {
                const diagnosticActions = await this.createActionsForDiagnostic(diagnostic, document, token);
    ...
    private createFileNotFoundActions(
        ...
        if (data.suggestions && data.suggestions.length > 0) {
            for (const suggestion of data.suggestions.slice(0, 3)) {
                const action = new CodeAction(`Replace with "${suggestion}"`, CodeActionKind.QuickFix);
    ...
    private createNamespaceNotFoundActions(
        ...
        if (data.relativePath) {
            const action = new CodeAction(`Use "minecraft:${data.relativePath}"`, CodeActionKind.QuickFix);
    ...
    private createInvalidFormatActions(
        ...
        if (/[A-Z]/.test(inputPath)) {
            ...
            const action = new CodeAction(`Convert to lowercase: "${fixedPath}"`, CodeActionKind.QuickFix);
        ...
        if (inputPath.includes('\\')) {
            ...
            const action = new CodeAction(`Fix path separators: "${fixedPath}"`, CodeActionKind.QuickFix);
        ...
        if (!inputPath.includes(':') && /^[a-z0-9_.-]+(?:\/[a-z0-9_.-]+)*$/.test(inputPath)) {
            ...
            const action = new CodeAction(`Add namespace: "${fixedPath}"`, CodeActionKind.QuickFix);
        ...
        if (cleanedPath !== inputPath && cleanedPath.length > 0) {
            const action = new CodeAction(`Clean path: "${cleanedPath}"`, CodeActionKind.QuickFix);
...
export function registerFilePathCommands(context: { subscriptions: { push(disposable: unknown): void } }): void {
    const logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger).createChild('FilePathCommands');
