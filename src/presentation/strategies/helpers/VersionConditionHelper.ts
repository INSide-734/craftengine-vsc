...
import { IMinecraftVersionService, IMinecraftVersion } from '../../../core/interfaces/IMinecraftVersionService';
import { ILogger } from '../../../core/interfaces/ILogger';
import { IDataConfigLoader, IVersionConditionConfig, IVersionConditionOperator } from '../../../core/interfaces/IDataConfigLoader';
...
export class VersionConditionHelper {
    ...
    constructor() {
        ...
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger)
            .createChild('VersionConditionHelper');
    }
    ...
    async createCompletionItems(options: VersionConditionCompletionOptions): Promise<CompletionItem[]> {
        ...
        try {
            ...
            const latestVersion = versions.find(v => v.isLatest)?.version || versions[0]?.version || defaults.fallbackVersion;
            ...
            // 操作符补全项
            items.push(...this.createOperatorItems(latestVersion, suffix, suffixDisplay, includeIdentifierSuffix, hasDoubleDoller));

            // 范围模式补全项
            items.push(this.createRangeItem(latestVersion, suffix, suffixDisplay, includeIdentifierSuffix, hasDoubleDoller));
    ...
    private createRangeItem(
        ...
        item.insertText = new SnippetString(
            `${prefix}\${1:1.20.1}~\${2:${latestVersion}}${suffix}`
        );
        item.detail = includeIdentifierSuffix 
     ...
     * 创建降级模板补全项
     */
    private createFallbackTemplateItem(isKeyPosition: boolean, includeIdentifierSuffix: boolean = false, hasDoubleDoller: boolean = false): CompletionItem {
     ...
     * 创建操作符文档
     */
    private createOperatorDoc(op: string, desc: string, example: string, includeIdentifierSuffix: boolean = false): MarkdownString {
