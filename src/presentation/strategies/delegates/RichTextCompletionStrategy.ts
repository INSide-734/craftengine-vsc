import { type CompletionItem, type CancellationToken } from 'vscode';
...
export class RichTextCompletionStrategy implements ICompletionStrategy {
    ...
    constructor() {
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger).createChild(
            'RichTextCompletionStrategy',
        );
        this.dataLoader = MiniMessageDataLoader.getInstance();

        const dataStoreService = ServiceContainer.getService<IDataStoreService>(SERVICE_TOKENS.DataStoreService);
