...
export class TranslationHandler {
     ...
     */
    async providePureTranslationKeyCompletions(linePrefix: string, mode: TranslationMode): Promise<CompletionItem[]> {
     ...
     * 搜索翻译键并创建补全项
     */
    private async searchAndCreateItems(prefix: string, mode: TranslationMode): Promise<CompletionItem[]> {
        try {
            ...
            return translationKeys.map((key: ITranslationKey) => this.createCompletionItem(key, mode));
        } catch (error) {
     ...
     * 创建翻译键补全项
     */
    private createCompletionItem(translationKey: ITranslationKey, mode: TranslationMode): CompletionItem {
    ...
    private validateTranslationKey(keys: ITranslationKey[]): TranslationValidationStatus {
        ...
        if (missingLanguages.length > 0 && missingLanguages.length <= 3) {
            status.hasWarnings = true;
            status.warnings.push(`Missing translations for common languages: ${missingLanguages.join(', ')}`);
        } else if (missingLanguages.length > 5) {
            status.hasErrors = true;
            status.errors.push(`Missing translations for ${missingLanguages.length} common languages`);
        }
