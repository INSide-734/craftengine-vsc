...
import { IMiniMessageCategoryConfig } from '../../../../core/types/ConfigTypes';
...
export class MiniMessageHandler {
    ...
    private createNegationHintItem(): CompletionItem {
        const decorations = this.negatableDecorations || ['bold', 'italic', 'underlined', 'strikethrough', 'obfuscated', 'shadow'];
        ...
        while ((match = tagPattern.exec(text)) !== null) {
            ...
            if (isClosing) {
                ...
                const tag = allTags.find(t =>
                    t.name === tagName || (t.aliases && t.aliases.includes(tagName))
                );
                if (!tag?.selfClosing) {
    ...
    private getSpecialTagArgumentCompletions(tagName: string, argIndex: number): CompletionItem[] {
        const specialCompletions: Record<string, () => CompletionItem[]> = {
            'click': () => argIndex === 0
                ? this.createEnumCompletions(this.dataLoader.getClickActions(), 'Click action')
                : [],
            'hover': () => argIndex === 0
                ? this.createEnumCompletions(this.dataLoader.getHoverActions(), 'Hover action')
                : [],
            ...
            'pride': () => argIndex === 0
                ? this.createEnumCompletions(this.dataLoader.getPrideFlags(), 'Pride flag')
                : [],
            'nbt': () => argIndex === 0
