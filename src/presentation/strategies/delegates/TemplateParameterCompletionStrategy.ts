import { CompletionItem, CompletionItemKind, MarkdownString, SnippetString, CancellationToken } from 'vscode';
...
export class TemplateParameterCompletionStrategy implements ICompletionStrategy {
    ...
    constructor() {
        ...
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger)
            .createChild('TemplateParameterCompletionStrategy');
     ...
     * 返回类型名称和已存在的属性
     */
    private detectExtendedTypeContext(context: ICompletionContextInfo): { type: string; existingProps: Set<string> } | null {
        const { document, position } = context;
