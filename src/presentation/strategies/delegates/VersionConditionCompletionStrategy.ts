...
export class VersionConditionCompletionStrategy implements ICompletionStrategy {
    ...
    constructor() {
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger)
            .createChild('VersionConditionCompletionStrategy');
    ...
    async provideCompletionItems(
        ...
        try {
            ...
            switch (stage.type) {
                ...
                case 'version':
                    return await this.provideVersionCompletions(
                        stage.operator || '', 
                        stage.partialVersion
                    );
     ...
     * 提供版本号补全
     */
    private async provideVersionCompletions(
        operator: string,
        partialVersion?: string
        ...
        if (partialVersion) {
            filteredVersions = versions.filter(v => 
                v.version.startsWith(partialVersion)
            );
        }
    ...
    private async provideRangeEndCompletions(startVersion: string): Promise<ICompletionResult> {
        ...
        const filteredVersions = versions.filter(v => 
            this.versionService.compareVersions(v.version, startVersion) > 0
        );
