import { window, Diagnostic, DiagnosticSeverity, Uri, TextDocument } from 'vscode';
...
export class ErrorNotificationManager {
    ...
    constructor() {
        this.logger = ServiceContainer.getService<ILogger>(SERVICE_TOKENS.Logger).createChild('ErrorNotificationManager');
    ...
    private async notifyCriticalErrors(uri: Uri, errors: Diagnostic[]): Promise<void> {
        ...
        const criticalErrors = errors.filter(e => 
            ...
            e.code === 'syntax-error' ||
            e.code === 'missing_required_parameter'
        );
    ...
    async showHintNotification(message: string, actions?: string[]): Promise<string | undefined> {
        if (actions) {
            return await window.showInformationMessage(`💡 ${message}`, ...actions);
        } else {
            window.showInformationMessage(`💡 ${message}`);
            return undefined;
        }
