import { type Uri, workspace, type Progress } from 'vscode';
import { type IYamlScanner, type IYamlScanOptions, type IYamlScanResult } from '../../core/interfaces/IYamlScanner';
import { type IYamlDocument } from '../../core/interfaces/IYamlDocument';
...
export class YamlScanner implements IYamlScanner {
    ...
    async scanWorkspace(
        ...
        try {
            ...
            const files = await workspace.findFiles(patternStr, excludeStr, undefined);
     ...
     * 扫描指定目录中的 YAML 文件
     */
    async scanDirectory(directory: Uri, options: IYamlScanOptions = {}): Promise<IYamlScanResult> {
        const pattern = options.pattern || '**/*.{yaml,yml}';
        const exclude = options.exclude || '**/node_modules/**';
        ...
        try {
            ...
            const files = await workspace.findFiles(relativePattern, excludeStr, undefined);
