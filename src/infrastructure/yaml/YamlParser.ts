import { type Uri, type TextDocument, Position, Range } from 'vscode';
import * as yaml from 'yaml';
import {
    type IYamlParser,
    type IYamlParseOptions,
    type IStreamParseOptions,
    type IIncrementalParseContext,
} from '../../core/interfaces/IYamlParser';
import { type IYamlDocument, type IYamlParseResult, type IYamlParseError } from '../../core/interfaces/IYamlDocument';
import { type ILogger } from '../../core/interfaces/ILogger';
import { YamlDocument, buildNodeTree, type PositionMap } from './YamlDocument';
...
export class YamlParser implements IYamlParser {
     ...
     * ```
     */
    async parseText(text: string, sourceFile: Uri, options: IYamlParseOptions = {}): Promise<IYamlParseResult> {
     ...
     * 解析 VSCode 文档
     */
    async parseDocument(document: TextDocument, options: IYamlParseOptions = {}): Promise<IYamlParseResult> {
    ...
    private buildPositionMapFromOffsets(
        ...
        const visitNode = (astNode: yaml.Node, path: (string | number)[] = []): void => {
            ...
            const rangeArray = Array.isArray(range) ? range : [range[0], range[1], range[2], range[3] as number];
            ...
            if (
                ...
                const position = convertRange([rangeArray[0], rangeArray[1], rangeArray[2], rangeArray[3]]);
