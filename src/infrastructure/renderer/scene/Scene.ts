...
import { GeometricalModel } from '../model/Model';
import { Cuboid } from './geometry/Cuboid';
...
export class Scene {
  ...
  async render(scaleWidth: number, scaleHeight: number): Promise<Buffer> {
    ...
    const cropW = Math.max(1, Math.min(
      Math.floor(currentWidth * (1 - 2 * this.cropHorizontal)),
      currentWidth - cropX
    ));
    const cropH = Math.max(1, Math.min(
      Math.floor(currentHeight * (1 - 2 * this.cropVertical)),
      currentHeight - cropY
    ));
