...
export class TexturedRectangle extends Rectangle {
    ...
    trace(ray: Ray): Intersection | null {
        ...
        // UV 到纹理坐标的映射
        const texX = Math.min(
            invertX ? Math.floor(texture.width - u * texture.width) : Math.floor(u * texture.width),
            texture.width - 1,
        );
        const texY = Math.min(Math.floor(texture.height - v * texture.height), texture.height - 1);
