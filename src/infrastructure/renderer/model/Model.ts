      ...
      for (let y = 0; y < layer.height; y++) {
        for (let x = 0; x < layer.width; x++) {
          ...
          if (dstA === 0) {
            ...
            result[dstIdx] = Math.round((layer.data[srcIdx] * srcA + result[dstIdx] * dstA * (1 - srcA)) / outA);
            result[dstIdx + 1] = Math.round((layer.data[srcIdx + 1] * srcA + result[dstIdx + 1] * dstA * (1 - srcA)) / outA);
            result[dstIdx + 2] = Math.round((layer.data[srcIdx + 2] * srcA + result[dstIdx + 2] * dstA * (1 - srcA)) / outA);
