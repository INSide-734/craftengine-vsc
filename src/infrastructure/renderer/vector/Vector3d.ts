    ...
    rotate(origin: Vector3d, axis: Axis, degrees: number): Vector3d {
        ...
        switch (axis) {
            case Axis.X: {
                ...
                return new Vector3d(x + origin.x, h * Math.cos(angle) + origin.y, h * Math.sin(angle) + origin.z);
            }

            case Axis.Y: {
                const angle = Math.atan2(x, z) + radians;
                const h = Math.sqrt(x * x + z * z);
                return new Vector3d(h * Math.sin(angle) + origin.x, y + origin.y, h * Math.cos(angle) + origin.z);
            }

            case Axis.Z: {
                const angle = Math.atan2(y, x) + radians;
                const h = Math.sqrt(x * x + y * y);
                return new Vector3d(h * Math.cos(angle) + origin.x, h * Math.sin(angle) + origin.y, z + origin.z);
            }
        }
    }
