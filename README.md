...
- **智能诊断系统**：实时检测模板使用错误
  - 检测缺少必需参数的情况
  - 检测缺少 `arguments` 部分的情况
  - 检测 `arguments` 部分中缺少特定参数的情况
- **可选参数警告**：提醒用户未覆盖的默认值参数
- **实时错误检查**：文档保存时自动检查模板使用错误
...
#### 🆕 新增功能
- **悬停提示功能**：将光标指向模板名称时显示详细的参数信息
- **定义跳转功能**：按住 Ctrl+左键点击模板名称可跳转到模板定义位置
- **智能识别**：精确识别模板名称边界，避免误触发

#### 🔧 改进
- 优化了模板解析器，支持记录模板定义的具体位置
- 提升了代码导航的准确性和响应速度
...
- **模板自动补全**：在 YAML 文件中输入 `template:` 时提供可用的模板建议
- **参数智能提示**：显示每个模板所需的参数
- **代码片段生成**：自动生成带有占位符的模板代码片段
- **实时文件监控**：自动扫描工作区中的 YAML 文件并更新模板缓存
- **Schema 验证**：为 YAML 文件提供动态生成的 JSON Schema
